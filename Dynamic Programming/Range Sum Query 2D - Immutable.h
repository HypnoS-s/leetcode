#pragma once
#include <vector>
using std::vector;


/*
Range Sum Query 2D - Immutable My Submissions Question
Total Accepted: 2605 Total Submissions: 13479 Difficulty: Medium
Given a 2D matrix matrix, find the sum of the elements inside the rectangle defined by its upper left corner (row1, col1) and lower right corner (row2, col2).

Range Sum Query 2D
The above rectangle (with the red border) is defined by (row1, col1) = (2, 1) and (row2, col2) = (4, 3), which contains sum = 8.

Example:
Given matrix = [
[3, 0, 1, 4, 2],
[5, 6, 3, 2, 1],
[1, 2, 0, 1, 5],
[4, 1, 0, 1, 7],
[1, 0, 3, 0, 5]
]

sumRegion(2, 1, 4, 3) -> 8
sumRegion(1, 1, 2, 2) -> 11
sumRegion(1, 2, 2, 4) -> 12
Note:
You may assume that the matrix does not change.
There are many calls to sumRegion function.
You may assume that row1 ¡Ü row2 and col1 ¡Ü col2.
Subscribe to see which companies asked this question

Hide Tags Dynamic Programming
Hide Similar Problems (E) Range Sum Query - Immutable
*/


class NumMatrix {
public:

	int* S;
	int row, col;
	NumMatrix(vector<vector<int>> &matrix) {
		row = matrix.size();
		col = row?matrix[0].size():0;
		S = new int[(row + 1)*(col+1)]  {};
		for (int i = 1; i <= row; i++)
		{
			int s = 0;
			for (int j = 1;j <= col;j++)
			{
				s += matrix[i-1][j-1];
				getS(i, j) = s + getS(i - 1, j);
			}
		}

	}
	int& getS(int i, int j)
	{
		return *(S + i*(col+1) + j);
	}
	int sumRegion(int row1, int col1, int row2, int col2) {
		return getS(row2 + 1, col2+1) - getS(row2 + 1, col1) - getS(row1, col2+1) + getS(row1, col1);
	}
};


// Your NumMatrix object will be instantiated and called as such:
// NumMatrix numMatrix(matrix);
// numMatrix.sumRegion(0, 1, 2, 3);
// numMatrix.sumRegion(1, 2, 3, 4);